...
- 라이트/다크 상태는 아이콘 변화와 눌림 상태로 읽히게 한다.

## `FloatingUtilityGroup`
언어 전환과 테마 전환을 담는 하단 플로팅 조작 묶음이다.

### 기준
- 화면 하단 우측에 고정한다.
- 모바일에서는 안전 영역을 침범하지 않아야 한다.
- 데스크톱에서는 본문 카드와 겹치지 않도록 여백을 확보한다.
- 두 조작은 같은 그룹 안에 두되, 공항 선택보다 항상 낮은 우선순위로 보이게 한다.
- 버튼 크기는 손가락 조작을 고려하되, 히어로 영역보다 확실히 작아야 한다.
- 스크롤 중에도 접근 가능해야 하지만, 화면의 주인공처럼 보이면 안 된다.
