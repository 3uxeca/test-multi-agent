...
- 언어와 테마는 하단 우측의 플로팅 조작으로 유지하되, 화면 가장자리와 겹치지 않도록 여백을 둔다.

### 데스크톱
- 헤더에는 공항 선택만 두고, 언어와 테마 전환은 본문 외곽의 우측 하단 플로팅 그룹에 둔다.
...
- 플로팅 그룹은 카드와 겹치지 않는 안전 위치를 확보하고, 시각적 우선순위는 공항 선택보다 낮게 둔다.
