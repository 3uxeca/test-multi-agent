...
- 상단 헤더에서는 공항 선택만 주 조작으로 남기고, `KO`/`EN`과 테마는 하단 플로팅 보조 조작으로 분리한다.
...
## 레이아웃 규칙
- 상단에는 공항 선택을 메인 조작으로 두고, 언어/테마 전환은 하단 플로팅 버튼 그룹으로 분리한다.
- 중앙에는 대기시간 히어로를 가장 큰 시각적 영역으로 둔다.
- 보조 정보는 히어로보다 약한 대비와 작은 면적을 사용한다.
- 모바일에서는 카드가 세로로 쌓이고, 데스크톱에서는 히어로와 보조 패널의 위계를 분리한다.
- 데스크톱에서는 공항 선택이 본문 시작부에서 바로 보이게 하고, 언어와 테마는 우측 하단의 고정형 플로팅 컨트롤로 둔다.
...
- 화면이 넓어져도 공항 선택의 위치는 유지하고, 언어/테마 전환은 하단 우측 플로팅 그룹의 기준점을 유지한다.
- 플로팅 그룹은 세로 화면에서는 하단 안전 영역 위에 걸치지 않도록 띄우고, 가로가 넓은 환경에서는 본문 카드와 겹치지 않도록 여백을 둔다.
- 플로팅 그룹은 스크롤 중에도 접근 가능해야 하지만, 메인 히어로나 체크포인트 패널보다 시각적으로 약해야 한다.
...
- 상태가 바뀌어도 공항 선택의 위치는 크게 바뀌지 않아야 하며, 언어/테마는 플로팅 그룹에 고정한다.
...
- 공항 선택은 헤더 안에서 읽히고, 언어 전환과 테마 전환은 플로팅 버튼으로 읽히게 한다.
